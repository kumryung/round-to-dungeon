...
*   **마을 시스템 (Town)**: 성, 방랑자 관리, 여관(휴식), 길드 영입, 창고 보관/정렬, 상점(소모품/장비/가챠), 대장간 크래프팅, 우편함 시스템 완비.
...
5.  **신규 마을 시스템 (여관/Inn)**: `innTab.js`를 신설하여 방랑자의 잃어버린 체력(HP)과 정신력을 골드와 시간에 비례해 회복시킬 수 있는 여관 시스템 구현 완료.
6.  **UI 컴포넌트 공통화 및 리팩토링**: `itemCardUtils.js`와 `buildingHeader.js`를 신설하여 분산되어 있던 마을 건물 헤더 및 아이템 카드 렌더링 로직을 재사용 가능한 컴포넌트로 분리.
7.  **길드 탭 UI 개선**: 길드의 미해금 슬롯 안내 텍스트를 "길드 레벨업 필요"에서 "👷 길드 Lv.n 필요"로 직관적으로 변경.
