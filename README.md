...
*   **스탯 상세**:
    *   `VIT`: 최대 체력 (1당 HP +5). `MaxHP = 50 + (VIT * 5)`.
...
*   **성장 시스템**:
    *   **EXP & 레벨업**: 몬스터 사냥 시 경험치 획득. 레벨업 시 **Free Stat Point** 지급.
    *   **스탯 분배**: 획득한 포인트로 원하는 스탯(VIT, STR 등) 강화 가능.
...
### 3.8. 기타 주요 시스템 (Misc Features)
*   **Fog of War (전장의 안개)**:
    *   플레이어 주변(기본 2칸)만 밝게 표시.
    *   지나온 길은 'Fog'(안개) 상태로, 미탐험 지역은 'Shroud'(암흑) 상태로 구분.
    *   **횃불** 사용 시 시야 +2칸 확장.
*   **반응형 디자인 (Mobile Support)**:
    *   데스크탑(3단), 태블릿(비율 조정), 모바일(세로 스택) 레이아웃 지원.
    *   터치 인터페이스에 최적화된 하단 인벤토리/메뉴 배치.
...
| 4.5 | Polishing | ✅ 완료 | EXP/레벨업, Fog of War, 반응형, SEO |

### 구현 완료된 추가 기능
- **EXP 시스템**: 경험치 바 구현, 레벨업 보상(스탯 포인트), 스탯 찍기 UI.
- **Fog of War**: 탐험 몰입감을 위한 시야 제한 시스템.
- **Responsive**: 모바일/태블릿 기기 대응 완료.
- **SEO**: 메타 태그(OG, Twitter Card) 적용.
